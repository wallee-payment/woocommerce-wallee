...
class WC_Wallee_Helper {
	...
	public function cleanup_line_items( array $line_items, $expected_sum, $currency ) {
		...
		if ( $has_coupons ) {
			$result = apply_filters( 'wc_wallee_packages_coupon_process_line_items_with_coupons', $line_items, $expected_sum, $currency ); //phpcs:ignore
		...
		if ( 0 !== (int) $inconsistent_amount ) {
			$enforce_consistency = get_option( WooCommerce_Wallee::WALLEE_CK_ENFORCE_CONSISTENCY );
			switch ( $enforce_consistency ) {
				case 'no':
					...
					$line_item->setName( esc_html__( 'Adjustment', 'woo-wallee' ) );
					...
					break;
				default:
					throw new WC_Wallee_Exception_Invalid_Transaction_Amount( esc_html( $effective_sum ), esc_html( $rounded_expected_sum ) );
			}
		}
	...
	public function lock_by_transaction_id( $space_id, $transaction_id ) {
		...
		$table_transaction_info = $wpdb->prefix . 'wallee_transaction_info';
	...
	public function get_clean_locale_for_string( $language_string, $use_default ) {
		...
		if ( strlen( $language_string ) >= 5 ) {
			...
			$language = WC_Wallee_Provider_Language::instance()->find( $language_string );
			if ( ! $language && strpos( $language_string, '-' ) !== false ) {
				...
				while ( ! $language && ! empty( $language_parts ) ) {
					$language = WC_Wallee_Provider_Language::instance()->find( implode( '-', $language_parts ) );
					array_pop( $language_parts );
				}
			}
		}
		if ( ! $language ) {
			...
			$language = WC_Wallee_Provider_Language::instance()->find_by_iso_code( $language_string );
		}
	...
	public function try_to_parse_date( $date_string ) {
		$date_of_birth = false;
		$custom_date_of_birth_format = apply_filters( 'wc_wallee_custom_date_of_birth_format', '' ); //phpcs:ignore
	...
	protected function restock_items_for_order( WC_Order $order ) {
		if (
			...
			&& $order && apply_filters( 'wallee_can_increase_order_stock', true, $order )//phpcs:ignore
			...
			foreach ( $order->get_items() as $item ) {
				...
				if ( $item->is_type( 'line_item' ) && $product && $product->managing_stock() ) {
					...
					if ( ! is_wp_error( $new_stock ) ) {
						...
						$order->add_order_note( sprintf( esc_html__( '%1$s stock increased from %2$s to %3$s.', 'woo-wallee' ), $item_name, $new_stock - $qty, $new_stock ) );
					}
				}
			}
			do_action( 'wc_wallee_restocked_order', $order ); //phpcs:ignore
		}
	}
