...
if ( ! class_exists( 'WooCommerce_Wallee' ) ) {
	...
	final class WooCommerce_Wallee {
		...
		public function secure_redirect_order_confirmed( $order_id ) {
			...
			if ( property_exists( $wc_transaction_info, 'get_transaction_id' ) ) {
				$state = $sdk_service_transaction->read( get_option( self::WALLEE_CK_SPACE_ID ), $wc_transaction_info->get_transaction_id() )->getState();
				if ( \Wallee\Sdk\Model\TransactionState::CONFIRMED === $state ) {
					wp_redirect($wc_service_transaction->get_payment_page_url(get_option(self::WALLEE_CK_SPACE_ID), $wc_transaction_info->get_transaction_id())); //phpcs:ignore
					exit;
				}
			}
		}
		...
		public static function migrate_plugin_data_on_activation() {
			$old_option_prefix = WC_Wallee_Migration::WALLEE_DEPRECATED_TABLE_PREFIX;
			$old_plugin_prefix = WC_Wallee_Migration::WALLEE_DEPRECATED_PLUGIN_PREFIX;

			$plugin_slug = $old_plugin_prefix . 'wallee'; // The slug of the old plugin.
			 ...
			 */
			if ( version_compare( $installed_version, self::WALLEE_UPGRADE_VERSION, '>=' ) ) {
				return;
			}

			global $wpdb;
			$options_to_migrate = [
				$old_option_prefix . WC_Wallee_Migration::WALLEE_CK_DB_VERSION,
				$old_option_prefix . WC_Wallee_Service_Manual_Task::WALLEE_CONFIG_KEY,
				$old_option_prefix . self::WALLEE_CK_SPACE_ID,
				$old_option_prefix . self::WALLEE_CK_SPACE_VIEW_ID,
				$old_option_prefix . self::WALLEE_CK_APP_USER_ID,
				$old_option_prefix . self::WALLEE_CK_APP_USER_KEY,
				$old_option_prefix . self::WALLEE_CK_CUSTOMER_INVOICE,
				$old_option_prefix . self::WALLEE_CK_CUSTOMER_PACKING,
				$old_option_prefix . self::WALLEE_CK_SHOP_EMAIL,
		...
		public function load_plugin_textdomain() {
			...
			load_plugin_textdomain( 'woo-wallee', false, plugin_basename( __DIR__ ) . '/languages' );
		}
		...
		public function register_order_statuses() {
			register_post_status(
				...
				array(
					...
					'label_count' => _n_noop( 'wallee Processing <span class="count">(%s)</span>', 'wallee Processing <span class="count">(%s)</span>', 'woo-wallee' ),
				)
			);
			register_post_status(
				'wc-wallee-waiting',
				array(
					...
					'show_in_admin_status_list' => true,
					/* translators: %s: replaces string */
					'label_count' => _n_noop( 'Waiting <span class="count">(%s)</span>', 'Waiting <span class="count">(%s)</span>', 'woo-wallee' ),
				)
			);
			register_post_status(
				'wc-wallee-manual',
				array(
					...
					'show_in_admin_status_list' => true,
					/* translators: %s: replaces string */
					'label_count' => _n_noop( 'Manual Decision <span class="count">(%s)</span>', 'Manual Decision <span class="count">(%s)</span>', 'woo-wallee' ),
				)
			);
		}
		...
		public function enqueue_javascript_script() {
			if ( is_cart() || is_checkout() ) {
				...
				$space_id = get_option( self::WALLEE_CK_SPACE_ID );
		...
		public function add_gateways( $methods ) {
			...
			$method_configurations = WC_Wallee_Entity_Method_Configuration::load_by_states_and_space_id(
				$space_id,
				array(
					WC_Wallee_Entity_Method_Configuration::WALLEE_STATE_ACTIVE,
					WC_Wallee_Entity_Method_Configuration::WALLEE_STATE_INACTIVE,
				)
			);
			try {
		...
		public function log( $message, $level = WC_Log_Levels::WARNING ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'Woocommerce Wallee: ' . $message ); //phpcs:ignore
			}
		}
	...
	function WC_Wallee_Blocks_Support() { //phpcs:ignore
		if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
			require_once __DIR__ . '/includes/class-wc-wallee-blocks-support.php';
